...
class DynamicStopLoss:
    ...
    def calculate_dynamic_stop_loss(self, symbol, entry_price, direction, df):
        ...
        try:
            ...
            logger.info(f"💰 وقف الخسارة لـ {symbol}: {stop_loss_price:.4f}")
...
class CompleteTradeManager:
    ...
    def manage_new_trade(self, trade_data):
        ...
        try:
            ...
            # ✅ إرسال إشعار بدء الإدارة مع معالجة الأخطاء
    ...
    def check_stop_loss(self, symbol, current_price):
        ...
        if should_close:
            logger.info(f"🚨 ينبغي إغلاق {symbol} بسبب {reason}")
            ...
            if success:
                ...
                # ✅ إرسال إشعار الإغلاق مع معالجة الأخطاء
                logger.info(f"📨 جاري إرسال إشعار إغلاق {symbol}...")
                notification_sent = self.send_trade_closed_notification(trade, current_price, reason, pnl_pct)
                if notification_sent:
                    logger.info(f"✅ تم إرسال إشعار إغلاق {symbol}")
                else:
                    logger.error(f"❌ فشل إرسال إشعار إغلاق {symbol}")
