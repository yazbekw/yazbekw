...
TAKE_PROFIT_LEVELS = {
    ...
    'LEVEL_2': {'target': 0.0024, 'allocation': 0.3},
    'LEVEL_3': {'target': 0.0025, 'allocation': 0.2}
}
    ...
    def calculate_dynamic_stop_loss(self, symbol, entry_price, direction, df):
        ...
        try:
            ...
            if direction == 'LONG':
                ...
                min_partial_distance = full_stop_distance * 0.6
                ...
                full_stop_loss = min(full_stop_loss, entry_price * 0.98)
                partial_stop_loss = min(partial_stop_loss, entry_price * 0.99)
            
                full_stop_loss = max(full_stop_loss, entry_price * 0.90)
                partial_stop_loss = max(partial_stop_loss, entry_price * 0.95)
                ...
                full_stop_loss = max(full_stop_loss, entry_price * 1.02)
                partial_stop_loss = max(partial_stop_loss, entry_price * 1.01)
            
                full_stop_loss = min(full_stop_loss, entry_price * 1.10)
                partial_stop_loss = min(partial_stop_loss, entry_price * 1.05)
...
class CompleteTradeManager:
    ...
    def sync_with_binance_positions(self):
        """مزامنة الصفقات مع Binance - مع تسجيل محسن"""
        try:
            logger.info("🔍 بدء مزامنة الصفقات مع Binance...")
            active_positions = self.get_active_positions_from_binance()
            
            if not active_positions:
                logger.info("🔍 لا توجد صفقات نشطة في Binance")
                return 0
            
            logger.info(f"🔍 وجدت {len(active_positions)} صفقة نشطة في Binance")
            ...
            for position in active_positions:
                ...
                if symbol not in current_managed:
                    logger.info(f"🔄 اكتشاف صفقة جديدة: {symbol} - الاتجاه: {position['direction']}")
                    ...
                    logger.debug(f"🔍 الصفقة {symbol} مُدارة بالفعل")
            ...
            logger.info(f"✅ انتهت المزامنة: أضيف {added_count}، أزيل {removed_count}، الإجمالي: {total_managed}")
            return added_count
    ...
    def check_stop_loss(self, symbol, current_price):
        ...
        min_distance = entry_price * RISK_SETTINGS['min_stop_loss_pct'] * 0.8
...
class TradeManagerBot:
    ...
    def start_management(self):
        """بدء إدارة الصفقات - محدثة وكاملة"""
        try:
            ...
            if margin_info:
                logger.info(f"✅ نسبة الهامش الأولية: {margin_info['margin_ratio']:.2%}")
            
            self.trade_manager.debug_active_positions()
            
            logger.info("🔄 بدء المزامنة الفورية الشاملة...")
            active_count = self.trade_manager.sync_with_binance_positions()
            logger.info(f"✅ تم اكتشاف {active_count} صفقة نشطة للمراقبة")
            
            if self.notifier and telegram_ok:
                message = (
                    f"🚀 <b>بدء تشغيل مدير الصفقات المتكامل - الإصدار المحسن</b>\n"
                    ...
                    f"🔍 كشف الصفقات: كل 30 ثانية\n"
                    f"📊 مراقبة الهامش: كل دقيقة\n"
                    f"📈 تقرير الأداء: كل 6 ساعات\n"
                    f"الصفقات المكتشفة: {active_count}\n"
    ...
    def management_loop(self):
        """حلقة الإدارة الرئيسية - مصححة وجاهزة"""
        last_report_time = datetime.now(damascus_tz)
        last_sync_time = datetime.now(damascus_tz)
        last_margin_check_time = datetime.now(damascus_tz)
        last_status_log_time = datetime.now(damascus_tz)
        last_margin_warning_time = datetime.now(damascus_tz)
        margin_warning_sent = False

        logger.info("🔄 مزامنة فورية مع صفقات Binance...")
        initial_sync_count = self.trade_manager.sync_with_binance_positions()
        logger.info(f"✅ تمت المزامنة الفورية لـ {initial_sync_count} صفقة")
        ...
        while True:
            try:
                ...
                if (current_time - last_sync_time).seconds >= 30:
                    logger.info("🔄 مزامنة دورية مع صفقات Binance...")
                    synced_count = self.trade_manager.sync_with_binance_positions()
                    if synced_count > 0:
                        logger.info(f"✅ تمت مزامنة {synced_count} صفقة جديدة")
                    last_sync_time = current_time
                
                closed_trades = self.trade_manager.check_managed_trades()
                if closed_trades:
                    logger.info(f"🔒 تم إغلاق {len(closed_trades)} صفقة: {closed_trades}")
                
                if (current_time - last_margin_check_time).seconds >= 60:
                    margin_health = self.trade_manager.margin_monitor.check_margin_health(self.trade_manager.client)
                    
                    if margin_health:
                        logger.info(f"📊 حالة الهامش: {margin_health['margin_ratio']:.2%}")
                        
                        if margin_health['is_risk_high']:
                            ...
                            if margin_warning_sent:
                                ...
                                safe_message = (
                                    f"✅ <b>عودة الهامش إلى الوضع الآمن</b>\n"
                                    f"نسبة الهامش المستخدم: {margin_health['margin_ratio']:.2%}\n"
                                    f"الرصيد المتاح: ${margin_health['available_balance']:.2f}\n"
                                    f"الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}"
                                )
                                self.notifier.send_message(safe_message)
                                margin_warning_sent = False
                    
                    last_margin_check_time = current_time
                
                if (current_time - last_report_time).seconds >= 21600:
                    logger.info("📊 إرسال تقرير الأداء الدوري...")
                    self.trade_manager.send_performance_report()
                    last_report_time = current_time
                
                if (current_time - last_status_log_time).seconds >= 600:
                    active_trades = len(self.trade_manager.managed_trades)
                    logger.info(f"📈 حالة البوت: {active_trades} صفقة مدارة")
                ...
                stop_message = (
                    f"🛑 <b>إيقاف مدير الصفقات</b>\n"
                    f"تم إيقاف البوت يدوياً\n"
                    f"الصفقات المدارة: {len(self.trade_manager.managed_trades)}\n"
                    f"الوقت: {datetime.now(damascus_tz).strftime('%Y-%m-%d %H:%M:%S')}"
                )
                ...
                error_message = (
                    f"⚠️ <b>خطأ في حلقة الإدارة</b>\n"
                    f"الخطأ: {str(e)[:100]}...\n"
                    f"جاري إعادة المحاولة بعد 30 ثانية\n"
                    f"الوقت: {datetime.now(damascus_tz).strftime('%H:%M:%S')}"
                )
...
def health_check():
    ...
    try:
        ...
        status = {
            'status': 'running',
            'version': 'enhanced_sync_v1',
            'managed_trades': len(bot.trade_manager.managed_trades),
            'sync_interval_seconds': 30,
            'margin_check_interval_seconds': 60,
            ...
            'features': [
                'dynamic_double_stop_loss',
                'continuous_sync_30s', 
                'real_time_margin_monitoring',
...
@app.route('/api/management/start-sync', methods=['POST'])
@require_api_key
def start_sync():
    """بدء المزامنة اليدوية للصفقات"""
    try:
        bot = TradeManagerBot.get_instance()
        ...
        return jsonify({
            'success': True,
            'message': f'تمت المزامنة اليدوية لـ {count} صفقة',
...
@app.route('/api/management/loop-status', methods=['GET'])
def get_loop_status():
    """الحصول على حالة حلقة الإدارة"""
    try:
        bot = TradeManagerBot.get_instance()
        
        status = {
            'status': 'running',
            'last_sync': datetime.now(damascus_tz).isoformat(),
            'managed_trades': len(bot.trade_manager.managed_trades),
            'sync_interval': '30 seconds',
            'margin_check_interval': '60 seconds',
...
def run_flask_app():
    ...
    try:
        ...
        logger.info(f"🌐 بدء تشغيل Flask على المنفذ {port}")
        app.run(host='0.0.0.0', port=port, debug=False, use_reloader=False)
    except Exception as e:
        logger.error(f"❌ خطأ في تشغيل Flask: {e}")

def main():
    """الدالة الرئيسية - محدثة وكاملة"""
    try:
        logger.info("🚀 بدء تشغيل مدير الصفقات المتكامل...")
        
        bot = TradeManagerBot.get_instance()
        
        logger.info("🔍 التحقق من اتصال APIs...")
        connection_ok = bot.test_connection()
        if not connection_ok:
            logger.error("❌ فشل الاتصال بـ Binance API")
            return
        
        logger.info("🔧 بدء إعداد نظام الإدارة...")
        success = bot.start_management()
        if not success:
            logger.error("❌ فشل بدء الإدارة، جاري الخروج...")
            return
        ...
        logger.info("✅ تم تهيئة النظام بالكامل")
        logger.info(f"🌐 تطبيق Flask يعمل على المنفذ: {os.environ.get('PORT', 10001)}")
        logger.info("⏰ مواعيد الفحص:")
        logger.info("  - 🔍 كشف الصفقات: كل 30 ثانية")
        logger.info("  - 📊 مراقبة الهامش: كل دقيقة") 
        logger.info("  - 📈 تقرير الأداء: كل 6 ساعات")
        logger.info("  - 💾 تسجيل الحالة: كل 10 دقائق")
        
        logger.info("🔄 بدء حلقة الإدارة الرئيسية...")
        bot.management_loop()
