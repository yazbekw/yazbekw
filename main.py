...
app = FastAPI(title="Crypto Market Phase Bot", version="8.4.0")
...
class TelegramNotifier:
    ...
    async def send_phase_alert(self, coin: str, analysis: Dict[str, Any], price: float, prices: List[float]):
        if analysis["confidence"] < self.confidence_threshold:
            safe_log_info(f"تم تخطي إشعار {coin}: الثقة منخفضة ({analysis['confidence']*100}%)", coin, "telegram")
            return False
        ...
        for attempt in range(3):
            ...
            if success:
                ...
                safe_log_info(f"تم إرسال إشعار لـ {coin} (ثقة: {confidence*100}%)", coin, "telegram")
                return True
...
@app.get("/")
async def root():
    return {"message": "بوت محسن لتحسين الدقة", "version": "8.4.0", "confidence_threshold": CONFIDENCE_THRESHOLD}
