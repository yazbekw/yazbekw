...
app = FastAPI(title="Crypto Market Phase Bot", version="9.0.0")

# إعدادات التلغرام والبيئة
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "")
CACHE_TTL = int(os.getenv("CACHE_TTL", 900))
CONFIDENCE_THRESHOLD = 0.85  # ⭐ قيمة ثابتة 85%
...
class MarketPhaseAnalyzer:
    """محلل مراحل السوق مع مؤشرات موزونة"""
    
    # ⭐ أوزان المؤشرات (1 = عادي, 1.5 = مهم, 2 = مهم جداً)
    INDICATOR_WEIGHTS = {
        'price_trend': 2.0,      # اتجاه السعر (مهم جداً)
        'volume_trend': 1.5,     # اتجاه الحجم (مهم)
        'rsi_signal': 1.5,       # إشارة RSI (مهم)
        'macd_signal': 1.3,      # إشارة MACD
        'volatility': 1.2,       # التقلبات
        'bb_signal': 1.2,        # إشارة بولينجر
        'support_resistance': 1.8, # مستويات الدعم والمقاومة
        'market_structure': 1.5,   # هيكل السوق
        'momentum': 1.3,         # الزخم
    ...
    def analyze_market_phase(prices: List[float], highs: List[float], lows: List[float], volumes: List[float], sentiment_score: float, coin_custom: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # تحليل المرحلة مع الأوزان
            phase_analysis = MarketPhaseAnalyzer._weighted_phase_analysis(
                latest, prev, sentiment_score, support, resistance, coin_custom
            )
    ...
    @staticmethod
    def _weighted_phase_analysis(latest, prev, sentiment_score: float, support: float, resistance: float, coin_custom: Dict) -> Dict[str, Any]:
        """تحليل المرحلة مع الأوزان"""
        ...
        markdown_signals = [
            ...
            (latest['volume_ratio'] > 1.2 and latest['close'] < prev['close'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['volume_trend']),
            (latest['rsi'] < 40, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['rsi_signal']),
            (latest['macd'] < latest['macd_signal'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['macd_signal']),
            (latest['close'] < latest['bb_lower'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['bb_signal']),
            (current_price < support, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['support_resistance']),
            (latest['volatility'] > vol_thresh + 0.02, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['volatility']),
            (sentiment_score < 0.3, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['sentiment'])
        ]
        ...
        markup_signals = [
            (latest['close'] > latest['sma20'] > latest['sma50'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['price_trend']),
            (latest['volume_ratio'] > 1.2 and latest['close'] > prev['close'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['volume_trend']),
            (latest['rsi'] > 50 and latest['rsi'] < rsi_high, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['rsi_signal']),
            (latest['macd'] > latest['macd_signal'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['macd_signal']),
            (latest['close'] > latest['bb_middle'], MarketPhaseAnalyzer.INDICATOR_WEIGHTS['bb_signal']),
            (current_price > resistance, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['support_resistance']),
            (latest['volatility'] < vol_thresh, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['volatility']),
            (sentiment_score > 0.6, MarketPhaseAnalyzer.INDICATOR_WEIGHTS['sentiment'])
        ]
        
        # 🟡 إشارات التجميع
        accumulation_signals = [
            (latest['volatility'] < vol_thresh, 1.2),
            (latest['volume_ratio'] < 0.8, 1.0),
            (latest['rsi'] < rsi_low, 1.5),
            (abs(latest['close'] - latest['sma20']) / latest['sma20'] < 0.03, 1.3),
            (latest['macd_hist'] > -0.1, 1.1),
            (current_price < support * 1.02, 1.4)
        ]
        
        # 🟠 إشارات التوزيع
        distribution_signals = [
            (latest['volatility'] > vol_thresh, 1.2),
            (latest['volume_ratio'] > 1.5, 1.3),
            (latest['rsi'] > rsi_high, 1.5),
            (abs(latest['close'] - latest['sma20']) / latest['sma20'] > 0.08, 1.3),
            (latest['macd_hist'] < 0, 1.1),
            (current_price > resistance * 0.98, 1.4)
        ]
        ...
        max_possible_score = sum(MarketPhaseAnalyzer.INDICATOR_WEIGHTS.values())
        ...
        return {
            ...
            "indicators": {
                ...
                "bb_position": "منخفض" if latest['close'] < latest['bb_lower'] else "مرتفع" if latest['close'] > latest['bb_upper'] else "وسط",
    ...
    def _get_action_recommendation(phase: str, confidence: float, current_price: float, support: float, resistance: float) -> str:
        ...
        if confidence > 0.8:
            if phase == "صعود":
                return f"🟢 شراء قوي - الهدف: ${resistance:,.2f}"
            elif phase == "هبوط":
                return f"🔴 بيع قوي - الدعم: ${support:,.2f}"
            elif phase == "تجميع":
                return f"🟡 استعداد للشراء - الدعم: ${support:,.2f}"
            elif phase == "توزيع":
                return f"🟠 استعداد للبيع - المقاومة: ${resistance:,.2f}"
            ...
            if phase == "صعود":
                ...
                return f"🟡 مراقبة للشراء"
            elif phase == "توزيع":
                return f"🟠 مراقبة للبيع"
...
class TelegramNotifier:
    ...
    def __init__(self, token: str, chat_id: str):
        ...
        self.confidence_threshold = CONFIDENCE_THRESHOLD  # ⭐ قيمة ثابتة 0.85
    ...
    async def send_phase_alert(self, coin: str, analysis: Dict[str, Any], price: float, prices: List[float]):
        ...
        message += f"⚡ v9.0 - مرشح: {self.confidence_threshold*100}%"
    ...
    def _generate_price_chart(self, prices: List[float], coin: str, support: float, resistance: float) -> str:
        try:
            ...
            plt.title(f"{coin.upper()} - تحليل السوق")
            plt.xlabel("الفترة")
...
@app.get("/")
async def root():
    return {"message": "بوت تحليل السوق المحسن", "version": "9.0.0", "confidence_threshold": CONFIDENCE_THRESHOLD}
...
async def startup_event():
    safe_log_info(f"بدء التشغيل - v9.0.0 - عتبة الثقة: {CONFIDENCE_THRESHOLD*100}%", "system", "startup")
