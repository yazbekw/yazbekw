...
TOTAL_POINTS = sum(INDICATOR_WEIGHTS.values())
...
SUPPORTED_COINS = {
    ...
    'matic': {'name': 'Polygon', 'coingecko_id': 'matic-network', 'binance_symbol': 'MATICUSDT', 'symbol': 'MATIC'},
    'link': {'name': 'Chainlink', 'coingecko_id': 'chainlink', 'binance_symbol': 'LINKUSDT', 'symbol': 'LINK'},
    'doge': {'name': 'Dogecoin', 'coingecko_id': 'dogecoin', 'binance_symbol': 'DOGEUSDT', 'symbol': 'DOGE'}
}
...
class TradingSignalAnalyzer:
    ...
    def _generate_detailed_signals(current_price: float, rsi: float, macd_data: Dict[str, float], 
        ...
        # 1. إشارة RSI (20 نقطة) - الوزن الجديد
        if rsi < 30:
            buy_points += INDICATOR_WEIGHTS["RSI"]  # 20 نقطة
        elif rsi < 40:
            buy_points += int(INDICATOR_WEIGHTS["RSI"] * 0.8)  # 16 نقطة
        elif rsi < 50:
            buy_points += int(INDICATOR_WEIGHTS["RSI"] * 0.5)  # 10 نقطة
        elif rsi > 70:
            sell_points += INDICATOR_WEIGHTS["RSI"]  # 20 نقطة
        elif rsi > 60:
            sell_points += int(INDICATOR_WEIGHTS["RSI"] * 0.8)  # 16 نقطة
        elif rsi > 50:
            sell_points += int(INDICATOR_WEIGHTS["RSI"] * 0.5)  # 10 نقطة

        # 2. إشارة MACD (20 نقطة) - الوزن الجديد
        if macd_data['histogram'] > 0.02 and macd_data['macd'] > macd_data['signal']:
            buy_points += INDICATOR_WEIGHTS["MACD"]  # 20 نقطة
        elif macd_data['histogram'] > 0:
            buy_points += int(INDICATOR_WEIGHTS["MACD"] * 0.75)  # 15 نقطة
        elif macd_data['histogram'] < -0.02 and macd_data['macd'] < macd_data['signal']:
            sell_points += INDICATOR_WEIGHTS["MACD"]  # 20 نقطة
        elif macd_data['histogram'] < 0:
            sell_points += int(INDICATOR_WEIGHTS["MACD"] * 0.75)  # 15 نقطة

        # 3. إشارة المتوسطات المتحركة (15 نقطة) - الوزن الجديد
        if current_price > ma_data['ema_8'] > ma_data['ema_21']:
            buy_points += INDICATOR_WEIGHTS["MOVING_AVERAGES"]  # 15 نقطة
        elif current_price > ma_data['ema_8']:
            buy_points += int(INDICATOR_WEIGHTS["MOVING_AVERAGES"] * 0.8)  # 12 نقطة
        elif current_price < ma_data['ema_8'] < ma_data['ema_21']:
            sell_points += INDICATOR_WEIGHTS["MOVING_AVERAGES"]  # 15 نقطة
        elif current_price < ma_data['ema_8']:
            sell_points += int(INDICATOR_WEIGHTS["MOVING_AVERAGES"] * 0.8)  # 12 نقطة

        # 4. إشارة Bollinger Bands (15 نقطة) - الوزن الجديد
        if current_price <= bb_data['lower']:
            buy_points += INDICATOR_WEIGHTS["BOLLINGER_BANDS"]  # 15 نقطة
        elif current_price >= bb_data['upper']:
            sell_points += INDICATOR_WEIGHTS["BOLLINGER_BANDS"]  # 15 نقطة
        elif bb_data['lower'] < current_price < bb_data['middle']:
            buy_points += int(INDICATOR_WEIGHTS["BOLLINGER_BANDS"] * 0.53)  # 8 نقطة
        elif bb_data['middle'] < current_price < bb_data['upper']:
            sell_points += int(INDICATOR_WEIGHTS["BOLLINGER_BANDS"] * 0.53)  # 8 نقطة

        # 5. إشارة Stochastic (15 نقطة) - الوزن الجديد
        if stoch_data['k'] < 20 and stoch_data['d'] < 20:
            buy_points += INDICATOR_WEIGHTS["STOCHASTIC"]  # 15 نقطة
        elif stoch_data['k'] < 30 and stoch_data['d'] < 30:
            buy_points += int(INDICATOR_WEIGHTS["STOCHASTIC"] * 0.67)  # 10 نقطة
        elif stoch_data['k'] > 80 and stoch_data['d'] > 80:
            sell_points += INDICATOR_WEIGHTS["STOCHASTIC"]  # 15 نقطة
        elif stoch_data['k'] > 70 and stoch_data['d'] > 70:
            sell_points += int(INDICATOR_WEIGHTS["STOCHASTIC"] * 0.67)  # 10 نقطة

        # 6. إشارة الحجم (15 نقطة) - الوزن الجديد
        if "متزايد" in volume_trend:
            if buy_points > sell_points:
                buy_points += INDICATOR_WEIGHTS["VOLUME"]  # 15 نقطة
            elif sell_points > buy_points:
                sell_points += INDICATOR_WEIGHTS["VOLUME"]  # 15 نقطة
        elif "متراجع" in volume_trend:
            if buy_points > sell_points:
                buy_points -= int(INDICATOR_WEIGHTS["VOLUME"] * 0.33)  # -5 نقطة
            elif sell_points > buy_points:
                sell_points -= int(INDICATOR_WEIGHTS["VOLUME"] * 0.33)  # -5 نقطة

        # حساب النسب النهائية بناء على المجموع الكلي للنقاط
...
@app.get("/indicator-weights")
async def get_indicator_weights():
    """endpoint جديد لعرض توزيع الأوزان"""
    return {
        "indicator_weights": INDICATOR_WEIGHTS,
        "total_points": TOTAL_POINTS,
        "weights_percentage": {k: f"{(v/TOTAL_POINTS)*100:.1f}%" for k, v in INDICATOR_WEIGHTS.items()}
    }

@app.on_event("startup")
async def startup_event():
    safe_log_info(f"بدء التشغيل - v5.0.0 - إشارات تداول 5 دقائق بتمييز واضح", "system", "startup")
    safe_log_info(f"توزيع الأوزان الجديد: {INDICATOR_WEIGHTS}", "system", "config")
    safe_log_info(f"المجموع الكلي للنقاط: {TOTAL_POINTS}", "system", "config")
