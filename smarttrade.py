...
class AdvancedFuturesBot:
    ...
    async def handle_price_sol(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """عرض سعر سولانا"""
        try:
            price = await self.trader.get_symbol_price('SOLUSDT')
            await self.send_telegram_message(update, f"💰 سعر `SOL`: `{price}` USDT")
        except Exception as e:
            await self.send_telegram_message(update, f"❌ خطأ في جلب سعر SOL: {str(e)}")

    # =========================================================================
    # المعالجات الحالية (يتم الحفاظ عليها مع إضافة الوقف التلقائي)
    # =========================================================================
...
/ping - فحص الحالة
    ...
    async def handle_price_bnb(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """عرض سعر BNB"""
        try:
            price = await self.trader.get_symbol_price('BNBUSDT')
            await self.send_telegram_message(update, f"💰 سعر `BNB`: `{price}` USDT")
        except Exception as e:
    ...
    async def handle_price_eth(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """عرض سعر ETH"""
        try:
            price = await self.trader.get_symbol_price('ETHUSDT')
            await self.send_telegram_message(update, f"💰 سعر `ETH`: `{price}` USDT")
        except Exception as e:
    ...
    async def handle_price_btc(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """عرض سعر BTC"""
        try:
            price = await self.trader.get_symbol_price('BTCUSDT')
            await self.send_telegram_message(update, f"💰 سعر `BTC`: `{price}` USDT")
        except Exception as e:
